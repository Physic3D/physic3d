...
qboolean CL_DispatchUserMessage( const char *pszName, int iSize, void *pbuf )
{
	...
	if( clgame.msg[i].func2 )
	{
		clgame.msg[i].func2( pszName, iSize, pbuf );
	}

	if( clgame.msg[i].func3 )
	{
		clgame.msg[i].func3( pszName, iSize, pbuf );
	}

	if( clgame.msg[i].func4 )
	{
		clgame.msg[i].func4( pszName, iSize, pbuf );
	}
...
void CL_ParseUserMessage( sizebuf_t *msg, int svc_num )
{
	...
	if( clgame.msg[i].func2 )
	{
		clgame.msg[i].func2( clgame.msg[i].name, iSize, pbuf );
	}

	if( clgame.msg[i].func3 )
	{
		clgame.msg[i].func3( clgame.msg[i].name, iSize, pbuf );
	}

	if( clgame.msg[i].func4 )
	{
		clgame.msg[i].func4( clgame.msg[i].name, iSize, pbuf );
	}
...
player_t g_Player[33];
int CL_ParseTeamInfo( const char *pszName, int iSize, void *pbuf )
{
	sizebuf_t _msg = { true, pszName, pbuf, 0, iSize * 8 };
	sizebuf_t *msg = &_msg;

	iIndex = BF_ReadByte( msg );
	szTeam = BF_ReadString( msg );

	if ( !CL_IsInGame( ) )
		return 0;

	if ( iIndex > 0 && iIndex <= CL_GetMaxClients( ) )
	{
		if ( Q_strstr( szTeam, "TERRORIST" ) )
		{
