...
void UI_Video_Menu( void )
{
	...
	if( !video.WasInit() )
	{
		...
		video.AddButton( L( "Video options" ), NULL, PC_VID_OPT, UI_VidOptions_Menu, QMF_NOTIFY );
		video.AddButton( L( "Video modes" ), NULL, PC_VID_MODES, UI_VidModes_Menu, QMF_NOTIFY );
		video.AddButton( L( "Done" ), NULL, PC_DONE, VoidCb( &CMenuFramework::Hide ), QMF_NOTIFY );
	}
